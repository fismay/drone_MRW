...
def main():
    ...
    try:
        ...
        # Вершины куба относительно текущего положения дрона (base_link)
        # Каждая точка задается как относительное смещение от ПРЕДЫДУЩЕЙ позиции
        cube_points = [
            # Нижняя грань куба (на высоте TAKEOFF_HEIGHT)
            (1.5, 0.0, 0.0),      # Вперед
            (0.0, 0.75, 0.0),      # Влево
            (-1.5, 0.0, 0.0),     # Назад
            (0.0, 0.75, 0.0),     # Вправо (возврат к начальной точке нижней грани)
            
            # Переход к верхней грани
            (1.5, 0.0, 0.0),           # Вверх
            
            # # Верхняя грань куба (на высоте TAKEOFF_HEIGHT + CUBE_HEIGHT)
            # (CUBE_SIDE_LENGTH, 0.0, 0.0),      # Вперед
            # (0.0, CUBE_SIDE_LENGTH, 0.0),      # Влево
            # (-CUBE_SIDE_LENGTH, 0.0, 0.0),     # Назад
            # (0.0, -CUBE_SIDE_LENGTH, 0.0),     # Вправо (возврат к начальной точке верхней грани)
            
            # Возврат в начальную точку
            # (0.0, 0.0, -CUBE_HEIGHT),          # Вниз (возврат в начальную точку)
        ]
